...
exit";
                ...
                // Write the batch file to a temp path
                string tempBat = Path.Combine(Path.GetTempPath(), "autopcchecker.bat");
                File.WriteAllText(tempBat, batchScript);

                // Setup to run with admin
                ProcessStartInfo psi = new ProcessStartInfo
                {
                    FileName = tempBat,
                    UseShellExecute = true,
                    Verb = "runas" // Forces admin mode
                };
            ...
            {
                MessageBox.Show("Error running admin batch: " + ex.Message);
            }
        }
